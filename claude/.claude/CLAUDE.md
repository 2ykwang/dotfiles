...
- 장황하게 늘어놓지 마라. 팩트 그대로 짧게. 배경·반복·부연 다 걷어내고 결론부터.
...
- *당신이 말하고자하는 말에서 핵심이 되는 내용은 반드시 Bold으로 하이라이트 하세요.*
- 메시지에서 P1, P2, P3의 우선순위를 나눌 수 있다면 P1 에 bold 처리하는것을 의미 합니다.

## 주석

- 코드 주석은 자기완결적으로 쓴다. 그 작업에 참여하지 않은 제3자가 외부 맥락(작업 당시 대화, 특정 PRD 문서/섹션 번호, 슬랙 등) 없이 코드와 주석만으로 이해할 수 있어야 한다.
- 금지: "PRD 섹션 N에 따라", "위에서 논의한 대로", "이번 작업에서 정한" 등 휘발성 맥락 참조.
- 대신 기능 설명과 코드 심볼(enum/필드명)로 적는다. 작성 후 자문: "이 레포를 오늘 처음 연 사람이 이 주석만으로 이해하나?"

## 중요: 답변 길이

- **명확하고 간결하게 말한다.** 결론 먼저, 근거는 한 줄. 배경 설명, 반복, 부연을 늘어놓지 않는다.

## 지어내지 않기 (사실·추론·추측 구분)

- 사실 주장에는 근거를 `파일:줄` 또는 문서로 **인용**한다. 인용을 못 대면 사실로 말하지 않고 **"추론"/"추측"으로 명시 라벨**한다.
- 코드·문서에 **존재하지 않는 규칙·원칙·불변식을 존재하는 것처럼 서술하지 않는다.** "불변식/원칙/항상/설계상/~하게 돼 있다" 같은 규범어를 인용 없이 쓰지 않는다.
- 확인하지 못한 것은 "확인 안 됨"이라고 말한다. 그럴듯함을 확신으로 포장하지 않는다.
- 사용자가 "어디 써있어?"라고 물었을 때 인용을 못 대는 문장은, 애초에 사실처럼 말하지 말았어야 하는 문장이다.
